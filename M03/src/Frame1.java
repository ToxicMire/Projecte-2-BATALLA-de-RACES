...
public class Frame1 extends JFrame {
    ...
    // BUTTONS
    // —————————————————————————————————————————————————————————————————————————————————————————————————————————
    // First of all, we create the buttons that we will use in "Frame1". It should
    // ask the user their name
    // ("usernameField") and let them play ("startButton") or exit the game
    // ("exitButton").
    ...
    public Frame1() {
        ...
        // First, we have to set a title, and we have chosen "BATTLE OF RACES:
        // Welcome!", because is the frame where
        // we will ask the player their name and what they want to do: play or exit the
        // game.
        ...
        // First, with "Toolkit.getDefaultToolkit().getScreenSize()" we can get the
        // screen's dimension of the computer
        // that the user's is playing from. We save the dimension in a variable named
        // "dim" and then we have to divide
        ...
        // USER'S SETTINGS
        // —————————————————————————————————————————————————————————————————————————————————————————————
        // Now that we have the window of the panel configured, we need to ask the
        // player what will their username be.
        // The "setFont" method is only to make a medieval aesthetic to the game. We
        // chose that font because it matches
        ...
        // START BUTTON
        // ————————————————————————————————————————————————————————————————————————————————————————————————
        // What we do here is add a new "ActionListener()" to indicate the button that
        // his action is execute "Frame".
        // First, we have to make the window invisible with the command
        // "setVisible(false)" so it doesn't open until
        // it has to. Then we [...]. Finally, we make the window visibel with the
        // command "setVisible(true)" so the
        ...
        // —————————————————————————————————————————————————————————————————————————————————————————————————
        // What we do here is add a new "ActionListener()" to indicate the button that
        // his action is closing the game
